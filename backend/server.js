...
app.get('/', (req, res) => {
  res.send('NeuroPath Engine Online');
});
...
app.post('/api/compiler/analyze', async (req, res) => {
  ...
  try {
    ...
    console.error("Lumina Connection Error:", error.message);
    res.status(500).json({ error: "Failed to connect to Lumina. Is Ollama running?" });
  }
});
...
SYSTEM: You are Lumina, a strict DSA tutor and silent static analyzer. Analyze the user's input and select the most appropriate operational mode. 
...
CURRENT TOPIC CONTEXT: ${topic || 'General DSA'}`;
  ...
  try {
    ...
    res.status(500).json({ reply: "System Error: Failed to connect to Lumina." });
  }
});
