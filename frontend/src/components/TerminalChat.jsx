...
export default function TerminalChat({ currentSpace, chatWidth, isZenMode, isZenChatOpen, onUpdateSpace, onOpenMindMap, onGenerateQuiz, chatScrollTrigger, topicData, whiteboardMode }) {
  ...
  const handleExport = async (format, length) => {
    ...
    try {
      ...
      if (length === 'summary') {
        ...
        content = `# NeuroPath Protocol // Full Transcript\n\n`;
        ...
        activeMessages.forEach(msg => {
          const roleName = msg.role === 'ai' ? 'LUMINA' : 'ANIII';
          content += `**${roleName}**:\n${msg.text}\n\n---\n\n`;
        });
      }
      ...
      if (format === 'md') {
        ...
        a.href = url;
        a.download = `NeuroPath_${length.toUpperCase()}_Export.md`;
        ...
        const opt = {
          margin:       0.5,
          filename:     `NeuroPath_${length.toUpperCase()}_Export.pdf`,
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const errorMsg = { role: 'ai', text: "System Error: Failed to connect to NeuroPath Engine." };
      updatedThreads = updatedThreads.map(t =>
